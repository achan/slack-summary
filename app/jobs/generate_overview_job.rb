class GenerateOverviewJob < ApplicationJob
  ...
  def build_prompt(grouped_by_channel, user_names)
    ...
    lines << "- Markdown formatted, 500 words maximum"
    lines << "- Group by theme, not by channel"
    lines << "- Use **bold** headers (not ## markdown headers) to separate 3-5 theme sections"
    lines << "- Prioritize: things worth knowing about > decisions made > operational issues > routine updates"
    lines << "- Name specific people when relevant"
    lines << "- Use the channel context below to weight your analysis — higher-priority channels"
    lines << "  and their activity deserve more attention and detail in the overview"
    lines << ""
    lines << "Exam/provider quality guidance:"
    lines << "- Focus on PATTERNS, not per-exam scores"
    lines << '- Examples: "Kristin Adams consistently skips blood thinner and chief complaint questions across multiple exams"'
    lines << '  or "Veronica Weatherspoon maintains perfect scores at high volume"'
    lines << "- Only call out providers whose performance is notably strong or weak"
    lines << "- Don't list every exam number and percentage"
    lines << ""
    lines << "Things worth knowing about:"
    lines << "- Surface notable events: outages, unusual errors, zero-score exams, patient no-shows, business developments"
    lines << "- Don't frame these as action items — just make sure they're visible in the overview"
    lines << ""
